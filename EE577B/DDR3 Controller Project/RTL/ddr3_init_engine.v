	...
	begin
		...
		// INIT Waits for 200 microseconds
			...
			`S_EMRS0: begin
				...
				a[3] <= 0; 					// burst type: sequential (nibble)
...
//****************try wl after MRS0
			...
			// `T_DQSL1-1: ts_con <= 2'b10;

			// `T_ODT_off-1: ts_con <= 0; 
...
//******************************************************************
		...
		endcase
		// if (counter == /*`T_EMRS1 + `tWLMRD + `tWLO*/`T_DQSH + 4)
		// begin
		// 	if(/*wl_flag==0 &&*/ dq[0]!=1) begin
		// 		counter <=`T_DQSH-2; 		
		// 	end
...
// buf u1(ts_con[0], ts_con_pre[0]);
// buf u2(ts_con[1], ts_con_pre[1]);

// wire clk_b1, clk_b2, clk_b3, clk_b4, clk_b5, clk_b6;

// buf cb1(clk_b1, clk);
// buf cb2(clk_b2, clk_b1);

// CLKBUF2 DELAY0 (.Y(clk_b1), .A(clk  ));
// CLKBUF2 DELAY1 (.Y(clk_b2), .A(clk_b1));
